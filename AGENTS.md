...
fallback and a set of C / Cython extensions built on top of a
vendored copy of `llhttp` under `vendor/llhttp/`.
...
| `aiohttp/_http_parser.pyx`                 | Cython HTTP parser bound to vendored llhttp                          |
...
| `aiohttp/hdrs.py` and `aiohttp/_headers.pyi` | known header constants; the `.pyi` is generated by `tools/gen.py`  |
| `vendor/llhttp/`                           | vendored llhttp source; rebuilt via `make generate-llhttp`           |
...
- One logical change per PR. If a refactor and a bugfix are bundled
  together, split them.
- Pre-commit hooks (the changelog filename check, ruff, mypy, codespell,
  yamllint, end-of-file-fixer, and others under
...
- The default branch is `master`, not `main`. Open PRs against
  `master`; backports to the active release branches (`3.12`, `3.13`,
  etc.) are handled automatically by `patchback` once the PR is
  merged and labelled.
...
- A bug fix in `aiohttp/http_parser.py` (pure-Python HTTP parser)
  usually needs a matching fix in `aiohttp/_http_parser.pyx` and
  possibly the vendored `vendor/llhttp/` sources. If the C path
  is genuinely unaffected (different code path, different
  invariant), say so explicitly in the "What do these changes do?"
  section.
...
```bash
make .develop      # installs deps, regenerates llhttp, builds Cython in place
...
## Cython, llhttp, and generated files
...
`make generate-llhttp` rebuilds the vendored llhttp sources under
`vendor/llhttp/`; only run it (and commit the result) if you actually
changed something under `vendor/llhttp/`.

`aiohttp/_headers.pyi` and `aiohttp/_find_header.c` are generated
...
- Pre-commit runs ruff, mypy, codespell, and the changelog
  filename check; let the hooks rewrite files in place rather
...
- Do not use em-dashes or sentence-separating dashes in PR prose
  or commit messages.
- Do not edit files under `vendor/llhttp/` by hand; regenerate
  via `make generate-llhttp` if you really need to bump the
  vendored copy, and split that bump into its own PR.
