...
| 3.8 | Fragment memory bound | (a) Declared byte size — `max_msg_size` enforced pre-FIN and at assembly (default 4 MiB). (b) The reader reassembles a frame arriving across many reads into a single growable `bytearray`, so retained memory is proportional to the payload (`max_msg_size` plus `bytearray` over-allocation headroom of up to ~10%) with no per-read object overhead regardless of read granularity. | **User**: set a smaller `max_msg_size` for protocols where messages are bounded (e.g. chat); the 4 MiB default suits arbitrary payloads. |
...
- **PR #13488** — reassembling a frame delivered across many small transport
  ...
  into a single growable `bytearray`, so retained memory is proportional to
  the payload (`max_msg_size` plus `bytearray` over-allocation headroom of up
  to ~10%) with no per-read object overhead and no re-copying of the
  accumulated prefix, regardless of read granularity.
