...
| 4.2 | Many small parts | `client_max_size` caps total bytes. `client_max_fields` (default `1000`, `0` disables) caps the number of form fields since PR #13738: urlencoded bodies are rejected by `yarl.query_to_pairs` before any pair is materialised, and since PR #13742 `Request.multipart()` passes it to `MultipartReader` as `max_parts`, so the reader raises before parsing the headers of the part past the limit, whether consumed via `Request.post()` or directly. Nested readers inherit the cap. Both paths raise `HTTPRequestEntityTooLarge`. | Direct `MultipartReader(...)` construction (e.g. `from_response` on the client) defaults to `max_parts=0`; callers that want a cap pass it explicitly. **User**: operators sensitive to live-object count should reduce `client_max_fields` and `client_max_size`. |
