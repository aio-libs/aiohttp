...
| # | Threat | Existing | Recommended |
| :--- | :--- | :--- | :--- |
| 3.1 | Unmasked client frames accepted | None — the reader is direction-agnostic; `web_ws.py` does not enforce client-mask either. | **Recommended hardening (Medium).** Add server-side enforcement: when the reader is created with a "I am a server" flag, reject any frame with `has_mask == 0` with a `PROTOCOL_ERROR`-style close. Conversely, the client reader should reject *masked* server frames per RFC 6455 §5.1. |
| 3.2 | Non-cryptographic mask RNG | `partial(random.getrandbits, 32)` per writer instance. | **Open question.** WebSocket masking exists for cache-poisoning resistance against intermediaries, not as a confidentiality primitive. PRNG may be good enough, or a switch to `secrets.token_bytes(4)` may be warranted. Decision deferred. |
