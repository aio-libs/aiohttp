...
async def test_ctor_with_cookies() -> None:
    # like similar to `test_web_response.test_response_cookies`
    # but cookies change in-place. (#4277)

    resp = web.HTTPException()
    assert str(resp.cookies) == ""
    resp.set_cookie("nickname", "Mike", max_age=30)
    assert dict(resp.headers) == {
        "Content-Type": "text/plain",
        "Set-Cookie": "nickname=Mike; Max-Age=30; Path=/",
    }
    assert str(resp.cookies) == "Set-Cookie: nickname=Mike; Max-Age=30; Path=/"
    assert resp.status == -1
    assert resp.text == "-1: "
    assert bool(resp)
    resp.del_cookie("nickname")
    assert (
        str(resp.cookies) == 'Set-Cookie: nickname=""; expires=Thu, '
...
class TestHTTPRequestEntityTooLarge:
    ...
    def test_pickle(self) -> None:
        resp = web.HTTPRequestEntityTooLarge(
            100, actual_size=123, headers={"X-Custom": "value"},
            reason="Too large"
        )
        resp.foo = "bar"
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
