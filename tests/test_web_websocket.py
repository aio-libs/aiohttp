...
async def test_websocket_prepared_property(make_request: _RequestMaker) -> None:
    ...
    # Even if _payload_writer is None, should still be prepared
    # (simulating timeout scenario where _payload_writer might not be set)
    ws._payload_writer = None
    assert ws.prepared  # Should still be True because _writer is set


async def test_websocket_prepared_with_timeout(make_request: _RequestMaker) -> None:
    """Test WebSocketResponse.prepared behavior in timeout scenarios."""
    req = make_request("GET", "/")
    ws = web.WebSocketResponse()

    # Mock the prepare process to simulate timeout
    with mock.patch.object(ws, "_post_start"):
        # Simulate partial preparation where _writer is set but _payload_writer might not be
        async def mock_prepare_timeout(request):
            # Set up the writer to simulate partial preparation
            ws._writer = mock.Mock()
            # Simulate timeout before _payload_writer is set
            raise asyncio.TimeoutError()

        with mock.patch.object(
            web.StreamResponse, "prepare", side_effect=mock_prepare_timeout
        ):
            try:
                await ws.prepare(req)
            except asyncio.TimeoutError:
                # Even after timeout, if _writer was set, prepared should be True
                assert ws.prepared

                # Should be safe to close when prepared is True
                ws._writer.close = mock.AsyncMock()
                await ws.close()  # This should not raise RuntimeError


async def test_websocket_not_prepared_close_error() -> None:
    """Test that closing an unprepared WebSocket raises RuntimeError."""
    ws = web.WebSocketResponse()

    # WebSocket is not prepared
    assert not ws.prepared

    # Attempting to close should raise RuntimeError
    with pytest.raises(RuntimeError, match="Call .prepare\\(\\) first"):
        await ws.close()


async def test_websocket_double_prepare_error(make_request: _RequestMaker) -> None:
    """Test that preparing WebSocket twice raises RuntimeError."""
    req = make_request("GET", "/")
    ws = web.WebSocketResponse()
    ...
    # Second prepare with only _writer set (not _payload_writer) should raise error
    ws._payload_writer = None  # Simulate scenario where _payload_writer is not set

    with pytest.raises(RuntimeError, match="WebSocket connection already prepared"):
        await ws.prepare(req)
