...
def test_parse_cookie_headers_multiple_in_header() -> None:
    """Test parsing multiple cookies in a single header."""
    # Multiple cookies with attributes
    header = "session=abc123; Path=/; user=john; Domain=.example.com"
    result = parse_cookie_headers([header])

    assert len(result) == 2

    # First cookie with its attribute
    assert result[0][0] == "session"
    assert result[0][1].value == "abc123"
    assert result[0][1]["path"] == "/"

    # Second cookie with its attribute
    assert result[1][0] == "user"
    assert result[1][1].value == "john"
    assert result[1][1]["domain"] == ".example.com"


def test_parse_cookie_headers_comma_separated() -> None:
    """Test cookies with commas (old Netscape style)."""
    # Comma becomes part of the first cookie's value
    result = parse_cookie_headers(["cookie1=value1, cookie2=value2"])
    assert len(result) == 2
    assert result[0][0] == "cookie1"
    assert result[0][1].value == "value1,"  # Comma included
    assert result[1][0] == "cookie2"
    assert result[1][1].value == "value2"


def test_parse_cookie_headers_case_sensitivity() -> None:
    """Test case sensitivity of names and attributes."""
    # Cookie names are case-sensitive
    result = parse_cookie_headers(["name=value1; Name=value2; NAME=value3"])
    assert len(result) == 3
    assert result[0][0] == "name"
    ...
    result = parse_cookie_headers(["name=value; PATH=/test; secure; HTTPONLY"])
    assert len(result) == 1
    assert result[0][1]["path"] == "/test"
    assert result[0][1]["secure"] is True
    assert result[0][1]["httponly"] is True
...
def test_parse_cookie_headers_empty_and_whitespace() -> None:
    """Test cookies with empty or whitespace values."""
    # Empty value
    result = parse_cookie_headers(["name="])
    assert len(result) == 1
    assert result[0][0] == "name"
    ...
    # Whitespace in quoted value is preserved
    result = parse_cookie_headers(['name="   "'])
    assert len(result) == 1
    assert result[0][1].value == "   "


def test_parse_cookie_headers_real_world() -> None:
    """Test real-world cookie examples."""
    # AWS ALB
    result = parse_cookie_headers(
        [
            "AWSALB=abcdefghijklmnopqrstuvwxyz0123456789ABCDEF; "
            "Expires=Wed, 09 Jun 2021 10:18:14 GMT; Path=/"
        ]
    )
    assert len(result) == 1
    assert result[0][0] == "AWSALB"
    assert result[0][1]["expires"] == "Wed, 09 Jun 2021 10:18:14 GMT"
    assert result[0][1]["path"] == "/"

    # Session with encoding
    result = parse_cookie_headers(
        [
            "session_id=s%3AabcXYZ123.signature123; Path=/; Secure; HttpOnly; SameSite=Strict"
        ]
    ...
    assert result[0][1].value == "s%3AabcXYZ123.signature123"  # Not decoded
    assert result[0][1]["secure"] is True
    assert result[0][1]["httponly"] is True
    assert result[0][1]["samesite"] == "Strict"

    # Load balancer with special chars
    result = parse_cookie_headers(["SERVERID=server-1|Xm1nY|Xm1nY; path=/"])
    assert len(result) == 1
    assert result[0][0] == "SERVERID"
    assert result[0][1].value == "server-1|Xm1nY|Xm1nY"
