...
from aiohttp.multidict import MutableMultiDict, CIMutableMultiDict
...
class TestWebRequest(unittest.TestCase):
    ...
    def test_ctor(self):
        ...
        get = req.GET
        self.assertEqual(MutableMultiDict([('a', '1'), ('b', '2')]), get)
    ...
    def test_content_type_from_spec(self):
        req = self.make_request(
            'Get', '/',
            CIMutableMultiDict([('CONTENT-TYPE', 'application/json')]))
        self.assertEqual('application/json', req.content_type)

    def test_content_type_from_spec_with_charset(self):
        req = self.make_request(
            'Get', '/',
            CIMutableMultiDict([('CONTENT-TYPE', 'text/html; charset=UTF-8')]))
    ...
    def test_content_length(self):
        req = self.make_request(
            'Get', '/',
            CIMutableMultiDict([('CONTENT-LENGTH', '123')]))
    ...
    def test_call_POST_on_GET_request(self):
        ...
        self.assertEqual(CIMutableMultiDict(), ret)

    def test_call_POST_on_weird_content_type(self):
        req = self.make_request(
            'POST', '/',
            headers=CIMutableMultiDict({'CONTENT-TYPE': 'something/weird'}))
    ...
    def test_request_cookie(self):
        headers = CIMutableMultiDict(COOKIE='cookie1=value1; cookie2=value2')
    ...
    def test_request_cookie__set_item(self):
        headers = CIMutableMultiDict(COOKIE='name=value')
        req = self.make_request('GET', '/', headers=headers)
