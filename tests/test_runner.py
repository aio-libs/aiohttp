...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        policy = _TestPolicy(self.new_loop)
...
class RunnerTests(BaseTest):
    ...
    def test_non_debug(self):
        with web.Runner(debug=False) as runner:
            self.assertFalse(runner.get_loop().get_debug())

    def test_debug(self):
        with web.Runner(debug=True) as runner:
            self.assertTrue(runner.get_loop().get_debug())

    def test_custom_factory(self):
        loop = mock.Mock()
        with web.Runner(loop_factory=lambda: loop) as runner:
