...
def test_connection_del(loop) -> None:
    ...
    msg = {"message": mock.ANY, "client_connection": mock.ANY}
...
async def test_tcp_connector_dns_tracing(loop, dns_response) -> None:
    ...
    on_dns_resolvehost_start = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
    on_dns_resolvehost_end = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
    on_dns_cache_hit = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
    on_dns_cache_miss = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
...
async def test_connect_tracing(loop) -> None:
    session = mock.Mock()
    trace_config_ctx = mock.Mock()
    on_connection_create_start = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
    on_connection_create_end = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
...
async def test_cleanup(key) -> None:
    testset = {key: [(mock.Mock(), 10), (mock.Mock(), 300)]}
...
async def test_connect_queued_operation_tracing(loop, key) -> None:
    session = mock.Mock()
    trace_config_ctx = mock.Mock()
    on_connection_queued_start = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
    on_connection_queued_end = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
...
async def test_connect_reuseconn_tracing(loop, key) -> None:
    session = mock.Mock()
    trace_config_ctx = mock.Mock()
    on_connection_reuseconn = mock.Mock(side_effect=asyncio.coroutine(mock.Mock()))
...
async def test_connect_with_limit_cancelled(loop) -> None:
    ...
    with pytest.raises(asyncio.TimeoutError):
        ...
        await asyncio.wait_for(
            conn.connect(req, None, ClientTimeout()), 0.01, loop=loop
        )
    connection.close()
...
async def test_tcp_connector_do_not_raise_connector_ssl_error(aiohttp_server) -> None:
    async def handler(request):
        return web.Response()
