...
def test_add_static_append_version(router: Any) -> None:
    resource = router.add_static("/st", Path(__file__).parent, name="static")
...
def test_add_static_append_version_set_from_constructor(router: Any) -> None:
    resource = router.add_static(
        "/st", Path(__file__).parent, append_version=True, name="static"
    )
...
def test_add_static_append_version_follow_symlink(router: Any, tmp_path: Any) -> None:
    ...
    symlink_target_path = Path(__file__).parent
    Path(str(symlink_path)).symlink_to(str(symlink_target_path), True)
...
def test_add_static_quoting(router: Any) -> None:
    resource = router.add_static(
        "/пре %2Fфикс", Path(aiohttp.__file__).parent, name="static"
    )
    assert router["static"] is resource
...
async def test_static_not_match(router: Any) -> None:
    router.add_static("/pre", Path(aiohttp.__file__).parent, name="name")
...
def test_static_repr(router: Any) -> None:
    router.add_static("/get", Path(aiohttp.__file__).parent, name="name")
    assert Matches(r"<StaticResource 'name' /get") == repr(router["name"])


def test_static_adds_slash(router: Any) -> None:
    route = router.add_static("/prefix", Path(aiohttp.__file__).parent)
...
def test_static_resource_get_info(router: Any) -> None:
    directory = Path(aiohttp.__file__).parent.resolve()
...
def test_static_route_user_home(router: Any) -> None:
    here = Path(aiohttp.__file__).parent
    try:
        static_dir = Path("~") / here.relative_to(Path.home())
    except ValueError:
...
def test_static_route_points_to_file(router: Any) -> None:
    here = Path(aiohttp.__file__).parent / "__init__.py"
    with pytest.raises(ValueError):
        router.add_static("/st", here)


async def test_404_for_static_resource(router: Any) -> None:
    resource = router.add_static("/st", Path(aiohttp.__file__).parent)
...
def test_static_resource_canonical() -> None:
    prefix = "/prefix"
    directory = str(Path(aiohttp.__file__).parent)
    canonical = prefix
